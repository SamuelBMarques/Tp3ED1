...
int H2(char * chave, int m) {
    ...
    return (soma % (m - 1)) + 1; // Garante que nunca seja 0
}
...
void inserirDocHash(DocHash *set, const char *doc) {
    int h1 = H((char*)doc, MAX_DOCS);
    int h2 = H2((char*)doc, MAX_DOCS);
    int j = 0;
    
    while (j < MAX_DOCS && set->ocupado[(h1 + j * h2) % MAX_DOCS]) {
        if (strcmp(set->documentos[(h1 + j * h2) % MAX_DOCS], doc) == 0) {
            return; // Já existe
        }
        j++;
    }
    if (j < MAX_DOCS) {
        strcpy(set->documentos[(h1 + j * h2) % MAX_DOCS], doc);
        set->ocupado[(h1 + j * h2) % MAX_DOCS] = 1;
    }
}
